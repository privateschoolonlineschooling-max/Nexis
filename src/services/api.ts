...
class ApiService {
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    const tryClientFallback = (): T | null => {
      try {
    ...
    try {
      ...
      // On network failure or connection issue, try client fallback first
      const fallbackResult = tryClientFallback();
    ...
    if (!res.ok) {
      // Automatic fallback for 405, 404 HTML, or 500+ server errors
      if (res.status === 405 || (res.status === 404 && contentType.includes('text/html')) || res.status >= 500) {
